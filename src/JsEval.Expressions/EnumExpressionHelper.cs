...
/// EnumStorage.AsString, EF Core with HasConversion&lt;string&gt;).
///
/// These helpers let you choose the comparison strategy explicitly.
/// </summary>
public static class EnumExpressionHelper
{
    /// <summary>
    ...
    /// // Produces: x => x.Status == TodoStatus.InProgress (no Convert, ORM decides)
    /// </code>
    /// </example>
    /// </summary>
    ...
    /// <summary>
    /// Compares enum as string from a typed enum value: <c>x.Status.ToString() == value.ToString()</c>.
    /// </summary>
    public static Expression<Func<T, bool>> EqualsAsString<T, TEnum>(string propertyName, TEnum value) where TEnum : struct, Enum
    {
        return EqualsAsString<T>(propertyName, value.ToString());
    }

    /// <summary>
    /// Compares enum as integer: <c>(int)x.Status == (int)value</c>.
    /// Use when the DB stores enums as their numeric value (default C#/EF Core behavior).
    /// </summary>
    ...
    /// <summary>
    /// Compares enum as integer from a string value: parses the string, then compares as int.
    /// </summary>
    ...
    /// <summary>
    /// Builds a string comparison: <c>member.ToString() == value</c>.
    /// </summary>
    ...
    /// <summary>
    /// Builds an integer comparison: <c>(int)member == (int)value</c>.
    /// </summary>
